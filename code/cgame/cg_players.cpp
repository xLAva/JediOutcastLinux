...
//	if (1)
	{
		...
		{//in first person or zoomed in
			...
			{//no viewentity
				if ( cent->currentState.number == cg.snap->ps.clientNum )
				{//I am the player
					if ((cg_useHmd.integer || cg.snap->ps.weapon != WP_SABER) && cg.snap->ps.weapon != WP_MELEE )
...
extern vmCvar_t	cg_thirdPersonAlpha;
		...
		if ( cent->currentState.number != 0 
			...
			|| ( !cg.renderingThirdPerson && ((!cg_useHmd.integer && cg.snap->ps.weapon == WP_SABER) || cg.snap->ps.weapon == WP_MELEE) )//First person saber
			)
