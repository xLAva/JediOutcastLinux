...
void *OvrSdk_0_5::oculus_library_handle;

#define OVRFUNC(need, rtype, fn, params)        \
rtype(*OvrSdk_0_5::d_ ## fn) params;
#include "ovr_dynamic_funcs.h"
#undef OVRFUNC

extern ovr_dynamic_load_result OvrSdk_0_5::oculus_dynamic_load(const char** failed_function) {
...
#define OVRFUNC(need, r, f, p)                                          \
    OvrSdk_0_5::d_##f = (pfn_##f)SDL_LoadFunction(oculus_library_handle, #f);       \
    if (need && !OvrSdk_0_5::d_##f) {                                               \
        if (failed_function)                                            \
            *failed_function = #f;                                      \
