...
void R_TransformDlights( int count, dlight_t *dl, orientationr_t *orLava) {
	...
	for ( i = 0 ; i < count ; i++, dl++ ) {
		VectorSubtract( dl->origin, orLava->origin, temp );
		dl->transformed[0] = DotProduct( temp, orLava->axis[0] );
		dl->transformed[1] = DotProduct( temp, orLava->axis[1] );
		dl->transformed[2] = DotProduct( temp, orLava->axis[2] );
	}
}
...
void R_DlightBmodel( bmodel_t *bmodel ) {
	...
	R_TransformDlights( tr.refdef.num_dlights, tr.refdef.dlights, &tr.orLava );
