...
Ghoul2 Insert End
*/
	int			damageValue;			//value at which to break
	struct shader_s *damageShader;		//switch to this shader when broken
...
typedef struct skin_s {
	...
	int			extensions;
	int			numSurfaces;
	skinSurface_t	*surfaces[MD3_MAX_SURFACES/2];	//the limit was raised to hold the whole dude, but we generally only skin half at a time
} skin_t;
...
#ifdef _NPATCH
	// Indication that model can be n-patched
	qboolean	 npatchable;
#endif // _NPATCH
...
#ifdef _NPATCH
// It seems from the description above that bit 2 isn't used
// so we reuse it to indicate n-patch capability
#define	QSORT_NPATCH_SHIFT		2
#endif // _NPATCH
...
typedef struct {
	...
	orientationr_t			orLava;					// for current entity
...
#ifdef _NPATCH
extern cvar_t	*r_ati_pn_triangles;    // controls n-patch extension
extern cvar_t	*r_npatches;			// controls n-patch tesselation level
#endif // _NPATCH
...
#ifdef _NPATCH
void R_AddDrawSurf( const surfaceType_t *surface, const shader_t *shader, int fogIndex, int dlightMap, int npatch );
#else
...
void R_RotateForEntity( const trRefEntity_t *ent, const viewParms_t *viewParms, orientationr_t *orLava );
...
void R_TransformDlights( int count, dlight_t *dl, orientationr_t *orLava );
