...
int R_CullLocalBox (vec3_t bounds[2]) {
	...
	for (i = 0 ; i < 8 ; i++) {
		...
		VectorCopy( tr.orLava.origin, transformed[i] );
		VectorMA( transformed[i], v[0], tr.orLava.axis[0], transformed[i] );
		VectorMA( transformed[i], v[1], tr.orLava.axis[1], transformed[i] );
		VectorMA( transformed[i], v[2], tr.orLava.axis[2], transformed[i] );
	}
...
void R_LocalNormalToWorld (vec3_t local, vec3_t world) {
	world[0] = local[0] * tr.orLava.axis[0][0] + local[1] * tr.orLava.axis[1][0] + local[2] * tr.orLava.axis[2][0];
	world[1] = local[0] * tr.orLava.axis[0][1] + local[1] * tr.orLava.axis[1][1] + local[2] * tr.orLava.axis[2][1];
	world[2] = local[0] * tr.orLava.axis[0][2] + local[1] * tr.orLava.axis[1][2] + local[2] * tr.orLava.axis[2][2];
}
...
void R_LocalPointToWorld (vec3_t local, vec3_t world) {
	world[0] = local[0] * tr.orLava.axis[0][0] + local[1] * tr.orLava.axis[1][0] + local[2] * tr.orLava.axis[2][0] + tr.orLava.origin[0];
	world[1] = local[0] * tr.orLava.axis[0][1] + local[1] * tr.orLava.axis[1][1] + local[2] * tr.orLava.axis[2][1] + tr.orLava.origin[1];
	world[2] = local[0] * tr.orLava.axis[0][2] + local[1] * tr.orLava.axis[1][2] + local[2] * tr.orLava.axis[2][2] + tr.orLava.origin[2];
}
...
void R_WorldToLocal (vec3_t world, vec3_t local) {
	local[0] = DotProduct(world, tr.orLava.axis[0]);
	local[1] = DotProduct(world, tr.orLava.axis[1]);
	local[2] = DotProduct(world, tr.orLava.axis[2]);
}
...
void R_RotateForEntity( const trRefEntity_t *ent, const viewParms_t *viewParms,
					   orientationr_t *orLava ) {
//	float	glMatrix[16];
	vec3_t	delta;
	float	axisLength;

	if ( ent->e.reType != RT_MODEL ) {
		*orLava = viewParms->world;
		return;
	}

	VectorCopy( ent->e.origin, orLava->origin );

	VectorCopy( ent->e.axis[0], orLava->axis[0] );
	VectorCopy( ent->e.axis[1], orLava->axis[1] );
	VectorCopy( ent->e.axis[2], orLava->axis[2] );

	preTransEntMatrix[0] = orLava->axis[0][0];
	preTransEntMatrix[4] = orLava->axis[1][0];
	preTransEntMatrix[8] = orLava->axis[2][0];
	preTransEntMatrix[12] = orLava->origin[0];

	preTransEntMatrix[1] = orLava->axis[0][1];
	preTransEntMatrix[5] = orLava->axis[1][1];
	preTransEntMatrix[9] = orLava->axis[2][1];
	preTransEntMatrix[13] = orLava->origin[1];

	preTransEntMatrix[2] = orLava->axis[0][2];
	preTransEntMatrix[6] = orLava->axis[1][2];
	preTransEntMatrix[10] = orLava->axis[2][2];
	preTransEntMatrix[14] = orLava->origin[2];
	...
	myGlMultMatrix( preTransEntMatrix, viewParms->world.modelMatrix, orLava->modelMatrix );

	// calculate the viewer origin in the model's space
	// needed for fog, specular, and environment mapping
	VectorSubtract( viewParms->orLava.origin, orLava->origin, delta );
	...
	orLava->viewOrigin[0] = DotProduct( delta, orLava->axis[0] ) * axisLength;
	orLava->viewOrigin[1] = DotProduct( delta, orLava->axis[1] ) * axisLength;
	orLava->viewOrigin[2] = DotProduct( delta, orLava->axis[2] ) * axisLength;
}
...
void R_RotateForViewer (void) 
{
	...
	memset (&tr.orLava, 0, sizeof(tr.orLava));
	tr.orLava.axis[0][0] = 1;
	tr.orLava.axis[1][1] = 1;
	tr.orLava.axis[2][2] = 1;
	VectorCopy (tr.viewParms.orLava.origin, tr.orLava.viewOrigin);

	// transform by the camera placement
	VectorCopy( tr.viewParms.orLava.origin, origin );

	viewerMatrix[0] = tr.viewParms.orLava.axis[0][0];
	viewerMatrix[4] = tr.viewParms.orLava.axis[0][1];
	viewerMatrix[8] = tr.viewParms.orLava.axis[0][2];
	viewerMatrix[12] = -origin[0] * viewerMatrix[0] + -origin[1] * viewerMatrix[4] + -origin[2] * viewerMatrix[8];

	viewerMatrix[1] = tr.viewParms.orLava.axis[1][0];
	viewerMatrix[5] = tr.viewParms.orLava.axis[1][1];
	viewerMatrix[9] = tr.viewParms.orLava.axis[1][2];
	viewerMatrix[13] = -origin[0] * viewerMatrix[1] + -origin[1] * viewerMatrix[5] + -origin[2] * viewerMatrix[9];

	viewerMatrix[2] = tr.viewParms.orLava.axis[2][0];
	viewerMatrix[6] = tr.viewParms.orLava.axis[2][1];
	viewerMatrix[10] = tr.viewParms.orLava.axis[2][2];
	...
	// convert from our coordinate system (looking down X)
	// to OpenGL's coordinate system (looking down -Z)
	myGlMultMatrix( viewerMatrix, s_flipMatrix, tr.orLava.modelMatrix );

	tr.viewParms.world = tr.orLava;
...
static void SetFarClip( void )
{
	...
	for ( i = 0; i < 8; i++ )
	{
		...
		VectorSubtract( v, tr.viewParms.orLava.origin, vecTo );
...
void R_SetupFrustum (void) {
	...
	VectorScale( tr.viewParms.orLava.axis[0], xs, tr.viewParms.frustum[0].normal );
	VectorMA( tr.viewParms.frustum[0].normal, xc, tr.viewParms.orLava.axis[1], tr.viewParms.frustum[0].normal );

	VectorScale( tr.viewParms.orLava.axis[0], xs, tr.viewParms.frustum[1].normal );
	VectorMA( tr.viewParms.frustum[1].normal, -xc, tr.viewParms.orLava.axis[1], tr.viewParms.frustum[1].normal );

	ang = tr.viewParms.fovY / 180 * M_PI * 0.5;
	xs = sin( ang );
	xc = cos( ang );

	VectorScale( tr.viewParms.orLava.axis[0], xs, tr.viewParms.frustum[2].normal );
	VectorMA( tr.viewParms.frustum[2].normal, xc, tr.viewParms.orLava.axis[2], tr.viewParms.frustum[2].normal );

	VectorScale( tr.viewParms.orLava.axis[0], xs, tr.viewParms.frustum[3].normal );
	VectorMA( tr.viewParms.frustum[3].normal, -xc, tr.viewParms.orLava.axis[2], tr.viewParms.frustum[3].normal );

	for (i=0 ; i<4 ; i++) {
		tr.viewParms.frustum[i].type = PLANE_NON_AXIAL;
		tr.viewParms.frustum[i].dist = DotProduct (tr.viewParms.orLava.origin, tr.viewParms.frustum[i].normal);
...
qboolean R_GetPortalOrientations( drawSurf_t *drawSurf, int entityNum, 
	...
	if ( entityNum != ENTITYNUM_WORLD ) {
		...
		R_RotateForEntity( tr.currentEntity, &tr.viewParms, &tr.orLava );

		// rotate the plane, but keep the non-rotated version for matching
		// against the portalSurface entities
		R_LocalNormalToWorld( originalPlane.normal, plane.normal );
		plane.dist = originalPlane.dist + DotProduct( plane.normal, tr.orLava.origin );

		// translate the original plane
		originalPlane.dist = originalPlane.dist + DotProduct( originalPlane.normal, tr.orLava.origin );
	} else {
		plane = originalPlane;
	}
...
static qboolean SurfIsOffscreen( const drawSurf_t *drawSurf, vec4_t clipDest[128] ) {
	...
	for ( i = 0; i < tess.numVertexes; i++ )
	{
		...
		R_TransformModelToClip( tess.xyz[i], tr.orLava.modelMatrix, tr.viewParms.projectionMatrix, eye, clip );
	...
	for ( i = 0; i < tess.numIndexes; i += 3 )
	{
		...
		VectorSubtract( tess.xyz[tess.indexes[i]], tr.viewParms.orLava.origin, normal );
...
qboolean R_MirrorViewBySurface (drawSurf_t *drawSurf, int entityNum) {
	...
	R_MirrorPoint (oldParms.orLava.origin, &surface, &camera, newParms.orLava.origin );

	VectorSubtract( vec3_origin, camera.axis[0], newParms.portalPlane.normal );
	newParms.portalPlane.dist = DotProduct( camera.origin, newParms.portalPlane.normal );
	
	R_MirrorVector (oldParms.orLava.axis[0], &surface, &camera, newParms.orLava.axis[0]);
	R_MirrorVector (oldParms.orLava.axis[1], &surface, &camera, newParms.orLava.axis[1]);
	R_MirrorVector (oldParms.orLava.axis[2], &surface, &camera, newParms.orLava.axis[2]);
...
void R_AddEntitySurfaces (void) {
	...
	for ( tr.currentEntityNum = 0; 
		...
		case RT_MODEL:
			// we must set up parts of tr.orLava for model culling
			R_RotateForEntity( ent, &tr.viewParms, &tr.orLava );
