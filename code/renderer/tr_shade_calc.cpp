...
static void GlobalVectorToLocal( const vec3_t in, vec3_t out ) {
	out[0] = DotProduct( in, backEnd.orLava.axis[0] );
	out[1] = DotProduct( in, backEnd.orLava.axis[1] );
	out[2] = DotProduct( in, backEnd.orLava.axis[2] );
}
...
static void AutospriteDeform( void ) {
	...
	if ( backEnd.currentEntity != &tr.worldEntity ) {
		GlobalVectorToLocal( backEnd.viewParms.orLava.axis[1], leftDir );
		GlobalVectorToLocal( backEnd.viewParms.orLava.axis[2], upDir );
	} else {
		VectorCopy( backEnd.viewParms.orLava.axis[1], leftDir );
		VectorCopy( backEnd.viewParms.orLava.axis[2], upDir );
	}
...
static void Autosprite2Deform( void ) {
	...
	if ( backEnd.currentEntity != &tr.worldEntity ) {
		GlobalVectorToLocal( backEnd.viewParms.orLava.axis[0], forward );
	} else {
		VectorCopy( backEnd.viewParms.orLava.axis[0], forward );
	}
...
void RB_CalcFogTexCoords( float *st ) {
	...
	VectorSubtract( backEnd.orLava.origin, backEnd.viewParms.orLava.origin, localVec );
	fogDistanceVector[0] = -backEnd.orLava.modelMatrix[2];
	fogDistanceVector[1] = -backEnd.orLava.modelMatrix[6];
	fogDistanceVector[2] = -backEnd.orLava.modelMatrix[10];
	fogDistanceVector[3] = DotProduct( localVec, backEnd.viewParms.orLava.axis[0] );
	...
	if ( fog->hasSurface ) {
		fogDepthVector[0] = fog->surface[0] * backEnd.orLava.axis[0][0] + 
			fog->surface[1] * backEnd.orLava.axis[0][1] + fog->surface[2] * backEnd.orLava.axis[0][2];
		fogDepthVector[1] = fog->surface[0] * backEnd.orLava.axis[1][0] + 
			fog->surface[1] * backEnd.orLava.axis[1][1] + fog->surface[2] * backEnd.orLava.axis[1][2];
		fogDepthVector[2] = fog->surface[0] * backEnd.orLava.axis[2][0] + 
			fog->surface[1] * backEnd.orLava.axis[2][1] + fog->surface[2] * backEnd.orLava.axis[2][2];
		fogDepthVector[3] = -fog->surface[3] + DotProduct( backEnd.orLava.origin, fog->surface );

		eyeT = DotProduct( backEnd.orLava.viewOrigin, fogDepthVector ) + fogDepthVector[3];
	} else {
...
void RB_CalcEnvironmentTexCoords( float *st ) 
{
	...
	if (backEnd.currentEntity && backEnd.currentEntity->e.renderfx&RF_FIRST_PERSON)	//this is a view model so we must use world lights instead of vieworg
	{
		...
		for (i = 0 ; i < tess.numVertexes ; i++, v += 4, normal += 4, st += 2 ) 
		{
			VectorSubtract (backEnd.orLava.viewOrigin, v, viewer);
