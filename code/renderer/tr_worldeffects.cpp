...
void CMistyFog::Update(CWorldEffectsSystem *system, float elapseTime)
{
	...
	forwardWind = DotProduct(mWindTransform, backEnd.viewParms.orLava.axis[0]);
	rightWind = DotProduct(mWindTransform, backEnd.viewParms.orLava.axis[1]);
...
void CWind::Update(CWorldEffectsSystem *system, float elapseTime)
{
	...
	VectorSubtract(backEnd.viewParms.orLava.origin, mPoint, difference);
...
//		mWindChange = 10;
	...
	VectorCopy(backEnd.viewParms.orLava.origin, origin);
...
void CSnowSystem::Render(void)
{
	...
	VectorAdd(backEnd.viewParms.orLava.origin, mMinSpread, origin);
...
//	float		percent;
	...
	VectorScale(backEnd.viewParms.orLava.axis[0], 1, forward);		// forward
	VectorScale(backEnd.viewParms.orLava.axis[1], 0.2f, left);		// left
	...
	qglMatrixMode(GL_MODELVIEW);
	qglPushMatrix();
    qglTranslatef (backEnd.viewParms.orLava.origin[0], backEnd.viewParms.orLava.origin[1],  backEnd.viewParms.orLava.origin[2]);
...
//	qglPushMatrix();
	...
	originContents = ri.CM_PointContents(backEnd.viewParms.orLava.origin, 0);
